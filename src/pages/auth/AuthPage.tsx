...
import megsyIconUrl from "@/assets/megsy-icon-transparent.png";
...
const AuthPage = () => {
  ...
  const handleCheckEmail = async () => {
    ...
    try {
      ...
      const isNetwork = /failed to (send|fetch)|network|load failed|networkerror/i.test(msg);
      if (isNetwork) {
        ...
        toast.error(translateAuthError(e, "couldNotCheckEmail"));
      }
    } finally {
  ...
  const handlePasswordLogin = async () => {
    ...
    try {
      ...
      if (/failed to (send|fetch)|network|load failed|networkerror/i.test(msg)) {
        ...
        if (userExists) {
          toast.error(authT("wrongPassword"));
        } else {
          toast.message(authT("noAccountFound"));
          try {
            await sendOTP(undefined, true);
            setStep("otp-signup");
          } catch {
            toast.error(translateAuthError(e, "loginFailed"));
          }
        }
      } else {
        toast.error(translateAuthError(e, "loginFailed"));
  ...
  const handleVerifyOTP = async (code: string) => {
    setIsSubmitting(true);
    try {
      const { data, error } = await invokeAuth({ action: "verify-otp", email: email.trim().toLowerCase(), code });
      if (error) throw new Error(error.message);
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      const normalizedEmail = email.trim().toLowerCase();
      const { data, error } = await invokeAuth({ action: "signup", email: normalizedEmail, password: newPassword });
      if (error) throw new Error(error.message);
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      const normalizedEmail = email.trim().toLowerCase();
      const { data, error } = await invokeAuth({ action: "update-password", email: normalizedEmail, password: newPassword });
      if (error) throw new Error(error.message);
  ...
  > = {
    intro1: { title: "", sub: "", index: 1, total: 1, label: "Get started" },
    email: {
    ...
    "forgot-password": {
      ...
      sub: `We'll send a verification code to ${email}`,
      index: 1,
      total: 3,
  ...
  // Primary CTA — solid brand color for high visibility
  const btnCls =
    "w-full py-3 rounded-full bg-primary text-primary-foreground text-[14px] font-semibold shadow-[0_0_20px_-4px_hsl(var(--primary)/0.45)] hover:shadow-[0_0_28px_-3px_hsl(var(--primary)/0.55)] hover:brightness-110 active:scale-[0.99] transition-all duration-150 disabled:opacity-40 disabled:pointer-events-none";
  ...
  // ─── Mobile-only redesigned flow (intro, email/password) ──
  if (isMobile && (step === "intro1" || step === "email" || step === "password")) {
    const mobileScreen: "intro" | "email" =
      step === "intro1" ? "intro" : "email";
    return (
      <>
        <SEOHead
          title="Sign in to Megsy AI"
          description="Sign in or create your Megsy AI account."
          path="/auth"
          noindex
        />
        <MobileAuthFlow
          ...
          onNextIntro={() => setStep("email")}
          onStartCreating={() => setStep("email")}
          ...
          onBack={() => {
            if (step === "password") {
              setPassword("");
              setStep("email");
            } else if (step === "email") setStep("intro1");
          }}
  ...
  return (
    <>
      <SEOHead
        ...
        noindex
      />
      <div className="relative min-h-dvh w-full overflow-hidden bg-background text-foreground flex flex-col lg:flex-row">
        {/* Cosmic backdrop */}
        <div className="absolute inset-0 -z-10">
          <div className="auth-bg absolute inset-0" />
          <div className="auth-stars absolute inset-0" />
          <div className="auth-bg-grain absolute inset-0" />
        </div>
