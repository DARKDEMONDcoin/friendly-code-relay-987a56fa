    ...
    if (triggerGate && !isPaidPlan) {
      ...
      setMessages((prev) => [
        ...
        } as Message,
        {
          ...
          content: `أهلاً 👋\n\n${arLabel} متاح للمشتركين فقط — الخدمة دي بتاكل موارد كبيرة وعشان كده محتاجين خطة مدفوعة.\n\nاشترك دلوقتي وافتح **توليد الصور** و**الفيديوهات** و**إنشاء المواقع** + نماذج برو بدون حدود.\n\n[✨ اشترك الآن](/billing)`,
          mode: chatMode,
        } as Message,
      ]);
      setInput("");
    ...
    return (
      <>
        ...
        <motion.div
          ...
          className={`hidden md:flex absolute left-2 ${hasConversation ? "bottom-full mb-2 origin-bottom-left" : "top-full mt-2 origin-top-left"} z-overlay w-[300px] rounded-2xl border border-white/10 bg-black/95 backdrop-blur-2xl shadow-[0_24px_48px_-12px_rgba(0,0,0,0.55)] overflow-y-auto overscroll-contain p-1.5 flex-col`}
        >
          {renderPlusContent()}
        </motion.div>
      </>
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex-1 flex flex-col min-w-0 relative overflow-hidden bg-transparent text-foreground">
          {/* Mobile animated blue gradient background */}
          <div className="md:hidden chat-mobile-blue-bg" aria-hidden="true" />
          ...
          {/* Mobile plus menu — single always-mounted vaul sheet (pure black, draggable) */}
          <GlassSheet open={plusMenuOpen && isMobileViewport} onOpenChange={setPlusMenuOpen}>
            <GlassSheetContent
              overlayClassName="bg-black/0 backdrop-blur-0"
              contentClassName="px-0 pt-1 bg-surface-1"
              className="bg-surface-1 border-t-[2.5px] border-brand-ink shadow-[0_-4px_0_rgba(59,130,246,0.18)]"
              style={{
            ...
            >
              <div ref={plusSheetScrollRef} data-plus-menu className="px-3 pt-1 pb-3 touch-pan-y">
                {renderPlusContent()}
              </div>
            </GlassSheetContent>
          </GlassSheet>
