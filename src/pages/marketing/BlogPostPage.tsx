...
const BlogPostPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase
        .from("blog_posts")
        .select("id,slug,title,meta_description,content_md,hero_image_url,keywords,category,reading_minutes,published_at,author_name,language,translation_group_id,faq")
        .eq("slug", slug)
        .eq("language", lang)
        .eq("status", "published")
        .maybeSingle();
  ...
  if (loading) {
    return (
      <div data-theme="dark" className="min-h-dvh bg-background text-foreground flex items-center justify-center">
  ...
  // hreflang alternates — always include x-default pointing at English original.
  const alts = useMemo(() => {
    const map = new Map<string, string>();
    for (const s of siblings) map.set(s.language, blogPath(s.slug, s.language));
    // Ensure current page is present
    if (!map.has(lang)) map.set(lang, blogPath(post.slug, lang));
    return Array.from(map.entries());
  }, [siblings, lang, post.slug]);

  const xDefaultHref =
    siblings.find((s) => s.language === "en")?.slug
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Blog", item: `${SITE_ORIGIN}${lang === "en" ? "/blog" : `/${lang}/blog`}` },
  ...
  return (
    <div data-theme="dark" dir={langMeta.dir} lang={lang} className="min-h-dvh overflow-x-hidden bg-background text-foreground">
      <SEOHead
        title={post.title}
        description={post.description}
        path={canonicalPath}
      ...
      <main className="px-4 pt-32 pb-20 mx-auto max-w-3xl">
        ...
        <header className="mt-8 mb-10">
          {(post as any).heroImage && (
            <img src={(post as any).heroImage} alt={post.title} className="w-full rounded-2xl mb-6 border border-border/60" loading="eager" />
          )}
          <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.16em] text-muted-foreground mb-4">
            <span>{post.category}</span>
            <span aria-hidden>·</span>
            <time dateTime={post.date}>
              {new Date(post.date).toLocaleDateString(lang, { month: "long", day: "numeric", year: "numeric" })}
            </time>
            <span aria-hidden>·</span>
        ...
        {siblings.length > 1 && (
          <nav aria-label="Languages" className="mb-10 flex flex-wrap gap-2">
            {siblings
              .slice()
              .sort((a, b) => BLOG_LANGS.findIndex((l) => l.code === a.language) - BLOG_LANGS.findIndex((l) => l.code === b.language))
              .map((s) => {
