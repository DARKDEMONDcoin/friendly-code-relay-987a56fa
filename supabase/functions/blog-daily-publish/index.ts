...
async function autoTopics(n: number): Promise<{ topic: string; angle?: string }[]> {
  ...
  const res = await fetch(llm.url, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: "system", content: "You suggest SEO-strong blog topics for Megsy AI (all-in-one AI platform for image, video, chat, code, slides). Topics must reflect 2026 reader intent, have search volume potential, and never duplicate evergreen clichés." },
        { role: "user", content: `Return JSON: { "topics": [{"topic":"...","angle":"..."}, ...${n} items] }.\nFocus mix: practical tutorials, comparison breakdowns, workflow guides, AI industry analysis. No generic listicles.` },
      ],
      temperature: 0.9,
  ...
  try {
    ...
    return Array.isArray(parsed.topics) ? parsed.topics.slice(0, n) : [];
  } catch { return []; }
}
